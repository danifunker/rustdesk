...
The app menu carries **About** — version, which CPU the copy was built for, and
the project's GitHub address with a button that opens it — and the icon is
generated by `deploy/make-icns.py`, which writes the `.icns` container itself
because there is no encoder on either machine: Leopard's `sips` refuses
`-s format icns`, `iconutil` is 10.7+, and Icon Composer is a GUI. Run that
script only when the icon changes; `app.icns` is committed.
