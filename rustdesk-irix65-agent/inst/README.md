...
- **`r-deskvint-irix.spec`** — product → image → one subsystem, named for the
...
- **`r-deskvint-irix.idb`** — the file list, **sorted by destination path**,
...
- **`usr/lib/r-deskvint-irix/libgcc_s.so.1`** is a private copy of the GCC
...
- **`usr/lib/X11/app-chests/R-DeskVint.chest`** is the Toolchest entry, a drop-in
